...
import { checkState, checkVideoState } from '@/api/request'
...
export async function handlePostVideoFiles(chunkList: string[], fileChunk: { file: Blob }[], uuid: string) {
    return new Promise((res, rej) => {
        const totalChunks = chunkList.length
        let successCount = 0
        const handle = () => {
            if (chunkList.length) {
                ...
                axios.post('/model/apply/uploadFile', formData, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                }).then(response => {
...
export async function checkVideoMergeStatus(key: string, callback: (res: number) => void) {
    async function handle() {
        const response = await new Promise(async (res, rej) => {
            res(await checkVideoMerge(key));
        });
        if (response === 0) {
            setTimeout(async () => {
                await handle();
...
export async function checkVideoMerge(key: string) {
    const state = await checkVideoState(key)
    return state.data
}
