<template>
    <el-form label-width="200px">
      <el-form-item label="上传实验问题视频" prop="file">
        <el-upload
          accept=".MP4, .mp4"
          ref="uploadRef"
          class="upload-demo"
...
import {
  mergePurposeVideoFiles,
  clearVideoTemp,
  removePurposeVideoFile
} from "@/api/request";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const deleteUpload = async () => {
      const data = await removePurposeVideoFile((router.currentRoute.value.params.apply as any).id);
    ...
    const submitUpload = async () => {
      ...
      const key = await mergePurposeVideoFiles(
