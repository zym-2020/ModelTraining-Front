  ...
  <div>
    <el-card>
      ...
      <div class="item" v-if="stepInfo.pictures.length > 0">
        <strong>图&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;片：</strong>
        <div class="text">
          <PicturesDownload :fileList="stepInfo.pictures" :fileName="'实验步骤'+stepInfo.name"></PicturesDownload>
        </div>
      </div>
...
import { getProcessLength, removeProcessVideoFile} from "@/api/request";
...
export default defineComponent({
  ...
  components: { InputOrOutputShow,ProcessVideoUpload,PicturesDownload },
  ...
  setup(props,context) {
    ...
    const deleteVideo = async () => {
      const data = await removeProcessVideoFile((router.currentRoute.value.params.apply as any).id,stepInfo.value);
      if (data != null && (data as any).code === 0) {
        notice("success", "成功", "删除成功！");
        processVideoItem.value.name = ''
        processVideoItem.value.id = ''
        stepInfo.value.video.name = ''
        stepInfo.value.video.id = ''
