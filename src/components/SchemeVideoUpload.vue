<template>
    <el-form label-width="200px">
      <el-form-item label="上传问题分析及设计视频" prop="file">
        <el-upload
          accept=".MP4, .mp4"
          ref="uploadRef"
          class="upload-demo"
...
import {
  mergeSchemeVideoFiles,
  clearVideoTemp,
  removeSchemeVideoFile
} from "@/api/request";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const deleteUpload = async () => {
      const data = await removeSchemeVideoFile((router.currentRoute.value.params.apply as any).id);
    ...
    const submitUpload = async () => {
      ...
      const key = await mergeSchemeVideoFiles(
