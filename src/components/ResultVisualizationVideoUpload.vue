<template>
    <el-form label-width="200px">
      <el-form-item label="上传结果可视化视频" prop="file">
        <el-upload
          accept=".MP4, .mp4"
          ref="uploadRef"
          class="upload-demo"
...
import {
  mergeResultVisualizationVideoFiles,
  clearVideoTemp,
  removeResultVisualizationVideoFile
} from "@/api/request";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const deleteUpload = async () => {
      const data = await removeResultVisualizationVideoFile((router.currentRoute.value.params.apply as any).id);
    ...
    const submitUpload = async () => {
      ...
      const key = await mergeResultVisualizationVideoFiles(
